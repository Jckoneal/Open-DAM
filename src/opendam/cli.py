...
def checkout(
    ...
    try:
        ...
        lock = locking.claim_lock(repo_path, info.path)
    ...
    console.print(f"[green]Checked out {info.name}[/green] — locked by you as of {lock.locked_at}.")
    console.print(f"Run 'dam checkin {info.name}' when done.")
    ...
    if not no_launch:
        ...
        try:
            ...
            launcher.launch(info.path, app_path)
        except OpenDamError as e:
...
def config_set(key: str, value: str, repo: str = typer.Option(".")) -> None:
    ...
    cfg = config_mod.Config.load(repo_path)
    parts = key.split(".")
    if len(parts) == 2 and parts[0] == "premiere":
