...
Extensions → Collaborate) that puts checkout/check-in buttons inside Premiere
...
| `collab list` | Table of all projects: name, lock status, holder, since (`--json` for machines) |
...
| `collab checkin <project>` | Commit, push, release the lock (`-m`, `--force-checkin`, `--keep-lock`, `--note "text"`, `--yes`) |
